...
async def search_by_image_historical(image_file: UploadFile = File(...)):
    try:
        ...
        search_object = SearchRequest([image_path], min_score=CONFIDENCE_THRESHOLD, search_mode=SearchMode.FAST, collection_id=HISTORICAL_COLLECTION_ID)
        results = sdk.search.search(search_object)
...
async def search_by_image_base64_historical(image_base64: str = Body(...)):
    try:
        ...
        search_object = SearchRequest([image_path], min_score=CONFIDENCE_THRESHOLD, search_mode=SearchMode.FAST, collection_ids=HISTORICAL_COLLECTION_ID)
        results = sdk.search.search(search_object)
